...
async function loadCloudUser(authUser) {
  ...
  state.user = { uid: authUser.uid, name: data.name || authUser.displayName || 'Utilisateur', email: authUser.email, photo: data.photo || null };
  ...
  try {
    ...
    if (authMode === 'signup') {
      try {
        ...
        showToast(`Compte créé, e-mail non envoyé : ${error.text || error.message}`);
...
async function sendWelcomeEmail(user) {
  if (!emailIsConfigured()) throw new Error('EmailJS n’est pas configuré');
  if (!window.emailjs) throw new Error('La bibliothèque EmailJS ne se charge pas');
  const config = window.CREDA_EMAIL_CONFIG;
  emailjs.init({ publicKey: config.publicKey });
  await emailjs.send(config.serviceId, config.templateId, { to_email: user.email, email: user.email, user_email: user.email, to_name: user.name, user_name: user.name, app_name: 'Creda', reply_to: user.email });
  return true;
}
...
function showDashboard() {
  ...
  $('profile-large-name').textContent = name; $('profile-email').textContent = state.user.email; $('profile-large-initials').textContent = initials(name); $('profile-large-initials').classList.toggle('hidden', Boolean(state.user.photo)); $('profile-photo-preview').classList.toggle('hidden', !state.user.photo); if (state.user.photo) $('profile-photo-preview').src = state.user.photo; renderProfileQr();
  $('client-count').textContent = `${new Set(state.debts.map(debt => debt.client)).size} client(s) enregistré(s)`;
  applySettings(); renderHome();
}
function renderProfileQr() { const target = $('profile-qr'); if (!target || !window.QRCode || !state.user) return; target.innerHTML = ''; new QRCode(target, { text: `${location.origin}${location.pathname}?user=${encodeURIComponent(state.user.email)}`, width: 128, height: 128, colorDark: '#162b2a', colorLight: '#ffffff' }); }
function clientDisplay(debt) { return debt.client || [debt.clientFirstName, debt.clientMiddleName, debt.clientLastName].filter(Boolean).join(' ') || 'Client sans nom'; }
function renderClientsView() { hideHome(); setActiveNav('nav-debts'); const clients = [...new Map(state.debts.map(debt => [clientDisplay(debt), debt])).values()]; $('feature-content').innerHTML = `<div class="feature-heading"><div><p class="eyebrow">CARNET CLIENT</p><h2>Clients</h2><p class="muted">Nom, coordonnées, profession et fidélité.</p></div><button class="button primary" id="clients-add">＋ Ajouter</button></div><div class="client-directory">${clients.length ? clients.map(debt => `<article class="client-card"><span class="avatar">${initials(clientDisplay(debt))}</span><div><strong>${clientDisplay(debt)}</strong><small>${debt.identification || 'Téléphone non renseigné'} · ${debt.clientType === 'fidele' ? 'Client fidèle' : 'Client passager'}</small><small>${debt.clientAddress || 'Adresse non renseignée'}${debt.clientProfession ? ` · ${debt.clientProfession}` : ''}</small></div></article>`).join('') : '<div class="empty-state"><strong>Aucun client</strong><span>Ajoutez une dette pour créer votre premier client.</span></div>'}</div>`; $('clients-add').addEventListener('click', openDebtModal); }
function renderPaymentsView() { hideHome(); setActiveNav('nav-debts'); const rows = state.debts.map(debt => `<article class="payment-card"><div><strong>${clientDisplay(debt)}</strong><small>${debt.product} · ${formatMoney(debt.amount, debt.currency)}</small></div><span class="status ${debtStatus(debt)}">${statusLabel(debtStatus(debt))}</span><button class="button secondary" data-pay="${debt.id || ''}">${debtStatus(debt) === 'settled' ? 'Payé' : 'Enregistrer un paiement'}</button></article>`).join(''); $('feature-content').innerHTML = `<div class="feature-heading"><div><p class="eyebrow">ENCAISSEMENTS</p><h2>Paiements</h2><p class="muted">Chaque paiement est enregistré et la dette est recalculée.</p></div></div><div class="payment-list">${rows || '<div class="empty-state"><strong>Aucun paiement</strong><span>Ajoutez une dette pour commencer.</span></div>'}</div>`; }
function renderReportsView() { hideHome(); setActiveNav('nav-reports'); const paid = state.debts.filter(debt => debtStatus(debt) === 'settled').length; const unpaid = state.debts.filter(debt => debtPaid(debt) === 0).length; const active = state.debts.filter(debt => debtPaid(debt) > 0 && debtStatus(debt) !== 'settled').length; const operations = [...state.debts.flatMap(debt => (debt.payments || []).map(payment => ({ label: `Paiement · ${clientDisplay(debt)}`, date: payment.date, amount: payment.amount, type: 'payment' }))), ...state.expenses.map(item => ({ label: `${item.type === 'income' ? 'Revenu' : 'Dépense'} · ${item.category}`, date: item.date, amount: item.amount, type: item.type }))].sort((a, b) => String(b.date).localeCompare(String(a.date))); $('feature-content').innerHTML = `<div class="feature-heading"><div><p class="eyebrow">VUE D'ENSEMBLE</p><h2>Rapports</h2><p class="muted">État des dettes et historique de vos opérations.</p></div><button class="button secondary" id="export-csv">↓ Exporter CSV</button></div><div class="report-grid"><div class="report-kpi"><small>Dettes payées</small><strong>${paid}</strong></div><div class="report-kpi"><small>Dettes impayées</small><strong>${unpaid}</strong></div><div class="report-kpi"><small>Dettes en cours</small><strong>${active}</strong></div></div><div class="report-panel"><h3>Opérations faites</h3><div class="operation-list">${operations.length ? operations.slice(0, 30).map(operation => `<div class="operation-row"><span>${operation.type === 'expense' ? '↘' : operation.type === 'income' ? '↗' : '✓'}</span><div><strong>${operation.label}</strong><small>${formatDate(operation.date)}</small></div><b>${formatMoney(operation.amount)}</b></div>`).join('') : '<p class="muted">Aucune opération enregistrée.</p>'}</div></div><div class="report-panel"><h3>Actions utiles</h3><p class="muted">Exportez toutes vos dettes, paiements et dépenses pour les archiver ou les partager.</p></div>`; $('export-csv').addEventListener('click', exportCsv); }
function renderExpensesView() { hideHome(); setActiveNav('nav-expenses'); const month = new Date().toISOString().slice(0, 7); const transactions = state.expenses.filter(item => item.date.startsWith(month)); const spent = transactions.filter(item => item.type === 'expense').reduce((sum, item) => sum + Number(item.amount), 0); const income = transactions.filter(item => item.type === 'income').reduce((sum, item) => sum + Number(item.amount), 0); $('feature-content').innerHTML = `<div class="feature-heading"><div><p class="eyebrow">FLUX DE TRÉSORERIE</p><h2>Dépenses & revenus</h2><p class="muted">${formatMoney(income)} de revenus · ${formatMoney(spent)} de dépenses ce mois.</p></div></div><form id="transaction-form" class="inline-form"><select id="transaction-type"><option value="expense">Dépense</option><option value="income">Revenu</option></select><input id="transaction-title" type="text" placeholder="Intitulé de la dépense" required><input id="transaction-amount" type="number" min="0" step="0.01" placeholder="Montant" required><select id="transaction-category"><option>Alimentation</option><option>Transport</option><option>Logement</option><option>Équipement</option><option>Santé</option><option>Autre</option></select><input id="transaction-note" type="text" placeholder="Note"><input id="transaction-date" type="date" required><button class="button primary" type="submit">Ajouter</button></form><div class="transaction-list">${transactions.length ? transactions.slice().reverse().map(item => `<div class="transaction-row"><span class="transaction-icon ${item.type}">${item.type === 'income' ? '↗' : '↘'}</span><div><strong>${item.title || item.category}</strong><small>${item.category} · ${item.note || 'Sans note'} · ${formatDate(item.date)}</small></div><b class="${item.type}">${item.type === 'income' ? '+' : '-'} ${formatMoney(item.amount, item.currency)}</b></div>`).join('') : '<div class="empty-state"><strong>Aucune transaction ce mois-ci</strong><span>Ajoutez votre première dépense ou votre premier revenu.</span></div>'}</div>`; $('transaction-date').value = new Date().toISOString().slice(0, 10); $('transaction-form').addEventListener('submit', event => { event.preventDefault(); state.expenses.unshift({ id: Date.now(), type: $('transaction-type').value, title: $('transaction-title').value.trim(), category: $('transaction-category').value, amount: $('transaction-amount').value, currency: state.settings.currency, date: $('transaction-date').value, note: $('transaction-note').value.trim() }); save(); renderExpensesView(); showToast('Transaction enregistrée'); }); }
...
$('debt-form').addEventListener('submit', event => { event.preventDefault(); const clientFirstName = $('client-first-name').value.trim(); const clientMiddleName = $('client-middle-name').value.trim(); const clientLastName = $('client-last-name').value.trim(); state.debts.unshift({ id: Date.now(), kind: $('debt-kind').value, urgency: $('urgency').value, client: [clientFirstName, clientMiddleName, clientLastName].filter(Boolean).join(' '), clientFirstName, clientMiddleName, clientLastName, clientAddress: $('client-address').value.trim(), clientProfession: $('client-profession').value.trim(), clientType: $('client-type').value, identification: $('identification').value.trim(), product: $('product').value.trim(), amount: $('amount').value, exchangeRate: $('exchange-rate').value, dueDate: $('due-date').value, currency: $('currency').value, note: $('note').value.trim(), payments: [] }); save(); event.target.reset(); $('exchange-rate').value = '1'; $('form-currency').textContent = 'USD'; $('rate-status').textContent = 'Ex. 1 USD = 2400 CDF'; $('debt-modal').classList.add('hidden'); renderHome(); showToast('Enregistrement ajouté'); });
...
$('profile-photo').addEventListener('change', event => { const file = event.target.files[0]; if (!file) return; if (file.size > 500000) return showToast('Choisissez une photo de moins de 500 Ko'); const reader = new FileReader(); reader.onload = () => { state.user.photo = reader.result; save(); showDashboard(); showToast('Photo de profil enregistrée'); }; reader.readAsDataURL(file); });
$('resend-welcome').addEventListener('click', async () => { try { await sendWelcomeEmail(state.user); showToast('E-mail de bienvenue renvoyé'); } catch (error) { showToast(`Envoi impossible : ${error.text || error.message}`); console.error('EmailJS error:', error); } });
$('clients-module').addEventListener('click', renderClientsView); $('payments-module').addEventListener('click', renderPaymentsView); $('reports-module').addEventListener('click', renderReportsView); $('nav-home').addEventListener('click', renderHome); $('nav-expenses').addEventListener('click', renderExpensesView); $('nav-debts').addEventListener('click', renderDebtsView); $('nav-reports').addEventListener('click', renderReportsView);
